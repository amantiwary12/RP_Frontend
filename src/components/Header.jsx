...
//                 <div className="absolute inset-0  rounded-lg blur-md opacity-60 group-hover:opacity-80 transition-opacity duration-500"></div>
//                 <div className="relative w-8 h-8 sm:w-10 sm:h-10 md:w-11 md:h-11 bg-gradient-to-br from-white to-gray-50 rounded-lg flex items-center justify-center border border-gray-200 group-hover:border-[#0AC4E0]/60 transition-all duration-300 transform shadow-sm">
...
//                   TRANSYNC
//                 </span>
//                 <span className="text-xs text-black font-bold hidden sm:block transition-colors duration-300">
...
export default function Header() {
 ...
 const checkAuth = async () => {
  ...
  // Try verification but don't block on CORS errors
  try {
    const response = await axios.get(
      "https://rp-875v.onrender.com/api/auth/verify",
      { 
    ...
    // Silently fail - already authenticated from stored data
    console.debug("Auth verification skipped - using stored credentials");
  }
};
  ...
  // Function to handle protected navigation
  const handleProtectedNavigation = (to, mobile = false) => {
    // Updated protected routes to include DR, EVENTS, and HISTORY
  ...
  const NavLink = ({ to, children, mobile = false }) => {
    ...
    if (mobile) {
      return (
        <a
          href={to}
          onClick={handleClick}
          className={`flex items-center py-4 px-6 transition-colors duration-200 border-l-2 ${
            isActive
              ? "bg-amber-500/10 text-white border-amber-500"
              : "text-slate-300 border-transparent hover:bg-slate-900 hover:border-amber-500/50"
          } ${requiresAuth ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
        >
          <div className={`w-2 h-2 rounded-full mr-3 ${
            isActive ? "bg-amber-500" : "bg-slate-600"
          }`}></div>
          <span className="font-semibold text-sm sm:text-base uppercase tracking-wide">{children}</span>
          {requiresAuth && (
            <span className="ml-2 text-xs bg-amber-500/10 text-amber-400 px-2 py-0.5 rounded border border-amber-500/30">
              Login Required
            </span>
          )}
        </a>
    ...
    return (
      <a
        href={to}
        onClick={handleClick}
        className={`relative py-2 px-3 lg:px-4 font-semibold uppercase tracking-wide transition-colors duration-200 ${
          isActive
            ? "text-amber-400 after:absolute after:bottom-0 after:left-3 after:right-3 after:h-0.5 after:bg-amber-500"
            : "text-slate-400 hover:text-white"
        } ${requiresAuth ? "opacity-50 cursor-not-allowed" : "cursor-pointer"} text-xs lg:text-sm`}
      >
        <span className="flex items-center">
          {children}
          {!isAuthenticated && isProtected && (
            <span className="ml-1.5 w-1.5 h-1.5 bg-amber-500 rounded-full"></span>
          )}
        </span>
      </a>
    );
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-50 w-full transition-all duration-300 bg-slate-950 border-b ${
          isScrolled
            ? "border-slate-800 shadow-lg shadow-black/40"
            : "border-slate-900"
        }`}
      >
        {/* Top accent bar */}
        <div className="h-[3px] w-full bg-amber-500"></div>
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8">
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            <Link
              ...
              onClick={() => setActiveNav("/")}
            >
              <div className="relative w-8 h-8 sm:w-10 sm:h-10 md:w-11 md:h-11 bg-amber-500 rounded-md flex items-center justify-center border border-amber-400/50 group-hover:bg-amber-400 transition-colors duration-200">
                <span className="text-base sm:text-lg md:text-xl font-black text-slate-950">TS</span>
              </div>
              <div className="flex flex-col leading-tight">
                <span className="text-base sm:text-lg md:text-xl font-black text-white tracking-tight">
                  TRANSYNC
                </span>
                <span className="text-[10px] sm:text-xs text-amber-500 font-bold uppercase tracking-widest hidden sm:block">
                  Bus Transfer System
                </span>
              </div>
            </Link>
            ...
            <nav className="hidden md:flex items-center space-x-1 lg:space-x-2">
              ...
              {/* Auth Section - Desktop */}
              <div className="flex items-center space-x-2 sm:space-x-3 ml-4 pl-4 border-l border-slate-800">
                {isAuthenticated ? (
                  <>
                    {user && (
                      <div className="group relative hidden sm:flex items-center space-x-2 bg-slate-900 rounded-md px-3 py-1.5 border border-slate-800">
                        <span className="w-2 h-2 bg-emerald-500 rounded-full"></span>
                        <span className="text-xs sm:text-sm text-slate-300 truncate max-w-[100px] lg:max-w-[120px] xl:max-w-[140px]">
                          {user.username}
                        </span>

                        {/* Tooltip */}
                        <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap border border-slate-700">
                          Logged in as {user.username}
                        </div>
                      </div>
                    )}
                    <button
                      onClick={handleLogout}
                      className="bg-red-600 text-white px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-bold uppercase tracking-wide hover:bg-red-500 transition-colors duration-200 border border-red-500/40 whitespace-nowrap"
                    >
                      <span className="flex items-center">
                        <svg className="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                  ...
                  <button
                    onClick={() => navigate("/login")}
                    className="bg-amber-500 text-slate-950 px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-bold uppercase tracking-wide hover:bg-amber-400 transition-colors duration-200 border border-amber-400/40 whitespace-nowrap"
                  >
                    <span className="flex items-center">
                      <svg className="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" />
            ...
            <nav className="hidden sm:flex md:hidden items-center space-x-1">
              ...
              {/* Tablet Auth Section */}
              <div className="flex items-center space-x-2 ml-3 pl-3 border-l border-slate-800">
                {isAuthenticated ? (
                  <>
                    {user && (
                      <div className="flex items-center space-x-2 bg-slate-900 rounded-md px-2 py-1 border border-slate-800">
                        <span className="w-2 h-2 bg-emerald-500 rounded-full"></span>
                    ...
                    <button
                      onClick={handleLogout}
                      className="bg-red-600 text-white px-2 py-1.5 rounded-md text-xs font-bold uppercase hover:bg-red-500 transition-colors duration-200 border border-red-500/40"
                    >
                      OUT
                    </button>
                  </>
                  ...
                  <button
                    onClick={() => navigate("/login")}
                    className="bg-amber-500 text-slate-950 px-2 py-1.5 rounded-md text-xs font-bold uppercase hover:bg-amber-400 transition-colors duration-200 border border-amber-400/40"
                  >
                    IN
                  </button>
                )}
            ...
            <div className="flex items-center md:hidden space-x-2">
              {/* Mobile Auth Status */}
              {isAuthenticated && user && (
                <div className="hidden xs:flex items-center space-x-2 bg-slate-900 rounded-md px-2 py-1.5 border border-slate-800">
                  <span className="w-2 h-2 bg-emerald-500 rounded-full"></span>
                  <span className="text-xs text-slate-300 truncate max-w-[80px] sm:max-w-[100px]">
                    {user.username}
                  </span>
                </div>
              )}
              ...
              <button
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className="relative p-2 rounded-md bg-slate-900 text-slate-300 hover:text-amber-400 focus:outline-none border border-slate-800 hover:border-amber-500/50 transition-colors duration-200"
                aria-label="Toggle menu"
              >
                {isMenuOpen ? (
                  <svg
      ...
      {isMenuOpen && (
        <>
          ...
          {/* Mobile Menu Panel */}
          <div className="fixed top-14 sm:top-16 right-0 w-full max-w-sm h-[calc(100vh-3.5rem)] sm:h-[calc(100vh-4rem)] bg-slate-950 border-l border-slate-800 shadow-2xl z-40 md:hidden overflow-y-auto animate-slideIn">
            {/* Top accent bar */}
            ...
            {isAuthenticated && user && (
              <div className="p-6 border-b border-slate-900 bg-slate-900/50">
                <div className="flex items-center space-x-4">
                  <div className="relative">
                    <div className="w-14 h-14 bg-amber-500 rounded-md flex items-center justify-center">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-amber-500 font-semibold uppercase tracking-wide flex items-center">
            ...
            <div className="border-t border-slate-900 mt-4 pt-4">
              ...
              {isAuthenticated ? (
                <>
                  <button
                    onClick={handleLogout}
                    className="mx-6 mb-4 flex items-center justify-center w-[calc(100%-3rem)] py-4 bg-red-600 text-white rounded-md text-sm font-bold uppercase tracking-wide hover:bg-red-500 transition-colors duration-200 border border-red-500/40"
                  >
                    <span className="flex items-center">
                      <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                ...
                <>
                  ...
                  <button
                    ...
                    className="mx-6 mb-6 flex items-center justify-center w-[calc(100%-3rem)] py-4 bg-amber-500 text-slate-950 rounded-md text-sm font-bold uppercase tracking-wide hover:bg-amber-400 transition-colors duration-200 border border-amber-400/40"
                  >
                    <span className="flex items-center">
                      <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" />
            ...
            {/* Footer Info */}
            <div className="absolute bottom-0 left-0 right-0 p-6 border-t border-slate-900 bg-slate-900/40">
              <div className="text-center text-xs font-bold uppercase tracking-widest text-slate-400">
                <span className="text-amber-500">TRANSYNC</span> v1.0 · Industrial Automation System
              </div>
